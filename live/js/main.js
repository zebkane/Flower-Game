...
class Player {
  ...
  draw() {
    ...
    if (this.animaton.rotate === 0) {
      ctx.drawImage(playerLeftImg, 162 * this.animaton.frame, 0, 162, 162, this.position.x, this.position.y, this.size.w, this.size.h);
    } else if (this.animaton.rotate === 1) {
      ctx.drawImage(playerDownImg, 162 * this.animaton.frame, 0, 162, 162, this.position.x, this.position.y, this.size.w, this.size.h);
    } else if (this.animaton.rotate === 2) {
      ctx.drawImage(playerRightImg, 162 * this.animaton.frame, 0, 162, 162, this.position.x, this.position.y, this.size.w, this.size.h);
    } else {
      ctx.drawImage(playerUpImg, 162 * this.animaton.frame, 0, 162, 162, this.position.x, this.position.y, this.size.w, this.size.h);
    }
    ...
    // ctx.drawImage(playerLeftImg, 162 * this.animaton.frame, 0, 162, 162, 0, 0, this.size.w, this.size.h);
    //
    // ctx.restore();
  ...
  move() {
    if (environment.input.keys.a) {
      ...
      this.animaton.timeWait = 5;
      this.animaton.rotate = 0;
    }
    if (environment.input.keys.d) {
      this.position.x += this.speed * deltaTime;
      this.animaton.timeWait = 5;
      this.animaton.rotate = 2;
    }
    if (environment.input.keys.s) {
      this.position.y += this.speed * deltaTime;
      this.animaton.timeWait = 5;
      this.animaton.rotate = 1;
    }
    if (environment.input.keys.w) {
      this.position.y -= this.speed * deltaTime;
      this.animaton.timeWait = 5;
      this.animaton.rotate = 3;
    }
    if (!environment.input.keys.w && !environment.input.keys.a && !environment.input.keys.s && !environment.input.keys.d) {
      this.animaton.timeWait = 10;
    }
  }

  change() {
    if (this.animaton.time >= this.animaton.timeWait) {
      this.animaton.frame++;
      this.animaton.time = 0;
      if (this.animaton.frame > 3) {
        this.animaton.frame = 0;
      }
    } else {
      this.animaton.time++;
    }
